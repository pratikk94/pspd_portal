...
app.get('/applications/:id', (req, res) => {
    ...
    connection.query('SELECT * FROM applications WHERE id = ?', id, (err, results) => {
        if (err) {
...
app.delete('/applications/:id', (req, res) => {
    ...
    connection.query('DELETE FROM applications WHERE id = ?', id, (err, result) => {
        if (err) {
...
app.post('/applications', upload.single('image'), (req, res) => {
    ...
    const sql = 'INSERT INTO applications (name, description, image, link, type_id) VALUES (?, ?, ?, ?, ?)';
...
app.put('/applications/:id', upload.single('image'), (req, res) => {
    ...
    const sql = 'UPDATE applications SET name=?, description=?, image=?, link=?, type_id=? WHERE id=?';
...
app.get('/applications', (req, res) => {
    connection.query('SELECT applications.*, type.type_name FROM applications LEFT JOIN type ON applications.type_id = type.type_id', (err, results) => {
        if (err) {
            console.error('Error fetching applications: ', err);
            res.status(500).send('Internal Server Error');
            return;
...
app.get('/applications/:id', (req, res) => {
    ...
    connection.query('SELECT applications.*, type.type_name FROM applications LEFT JOIN type ON applications.type_id = type.type_id WHERE applications.id = ?', id, (err, results) => {
        if (err) {
            console.error('Error fetching application: ', err);
            res.status(500).send('Internal Server Error');
            return;
